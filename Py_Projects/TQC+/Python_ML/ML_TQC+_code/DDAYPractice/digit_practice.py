...
for name, model in zip(lst_name,lst_model):
    ...
    print(name, 'acc: ', accuracy_score(y_digits, pred).round(4))
...
print('Accuracy=    ',  accuracy_score(y_digits,pred2).round(4)               )
...
for name,model in zip(lst_name, lst_model):
    ...
    print(name, 'ss', ss(x_std, model.labels_, metric='euclidean').round(4))
    print(name,'acc', acc(y_digits, y_pred).round(4))
...
reduct = PCA(n_components=2, random_state=seed).fit_transform(x_std)
kmeans.fit(reduct)
y_pred2 = kmeans.predict(reduct)
print('PCA+KMeans Silhouette=   ', ss(reduct, kmeans.labels_).round(4)      )
print('Accuracy=    ', acc(y_digits, y_pred2).round(4)                 )
