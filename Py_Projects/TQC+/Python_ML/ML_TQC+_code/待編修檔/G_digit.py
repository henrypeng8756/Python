...
dig = load_digits()
# 載入手寫數字資料集
X_digits = dig.data #TODO 數值特徵
y_digits = dig.target #TODO 數字類別
...
# TODO
pca = PCA(n_components=n_digits, random_state=seed).fit(xstd)
...
for name,model in zip(lst_name,lst_model):
    model.fit(xstd)
    pred = model.predict(xstd)
    print("ss= %.4f" % silhouette_score(xstd,model.labels_,metric='euclidean'))
    print("acc : %.4f" % accuracy_score(y_digits,model.labels_))
...
reduct = PCA(n_components=2, random_state=seed).fit_transform(xstd)
kmeans.fit(reduct)
pred2 = kmeans.predict(reduct)
print('PCA+KMeans Silhouette= %.4f' % silhouette_score(reduct,kmeans.labels_))
print('Accuracy= %.4f' % accuracy_score(y_digits,pred2))
